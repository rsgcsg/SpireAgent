...
import { buildHumanEnvironmentAllowedActions } from "../src/domain/actions/buildHumanEnvironmentAllowedActions.js";
import type { AdapterDescriptor } from "../src/game-io/adapter.js";
import { Sts2HumanEnvironmentAdapter } from "../src/integrations/sts2mcp/humanEnvironmentAdapter.js";
...
} from "../src/integrations/sts2mcp/humanEnvironmentProtocol.js";
import { wrapHumanEnvironmentState } from "../src/integrations/sts2mcp/rawState.js";
import { normalizeHumanEnvironmentCurrentState } from "../src/normalization/normalizeHumanEnvironmentCurrentState.js";
...
describe("Human Environment C", () => {
  ...
  it("projects current bound actions to opaque Re choices without V2 legal_actions wire data", () => {
    const raw = wrapHumanEnvironmentState({ snapshot: snapshot() });
    const normalized = normalizeHumanEnvironmentCurrentState(raw, SOURCE, "2026-08-04T00:00:00Z");
    expect(normalized.currentState.surface.kind).toBe("human_ui");
    const actions = buildHumanEnvironmentAllowedActions(normalized.currentState, normalized.stateHash);
  ...
  it("keeps semantic observation valid but withholds Re authority for a truncated finite projection", () => {
    ...
    const normalized = normalizeHumanEnvironmentCurrentState(
      wrapHumanEnvironmentState({ snapshot: current }),
      SOURCE,
    ...
    expect(buildHumanEnvironmentAllowedActions(normalized.currentState, normalized.stateHash)).toEqual([]);
  });
  ...
  it("uses the Human Environment control schema without a V3 wire dependency", () => {
  ...
  it("rejects an observation from a different exact environment", async () => {
    ...
    const adapter = new Sts2HumanEnvironmentAdapter(
  ...
  it("keeps a truncated observation readable while withholding adapter invocations", async () => {
    ...
    const normalized = normalizeHumanEnvironmentCurrentState(raw, adapter.describe());
    await adapter.close();
  ...
  it("treats applied HE input as delivered while successor readiness remains separate", async () => {
    ...
    const raw = await adapter.readCurrentState();
    const normalized = normalizeHumanEnvironmentCurrentState(raw, adapter.describe());
    const action = buildHumanEnvironmentAllowedActions(normalized.currentState, normalized.stateHash)[0]!;
