...
import {
  ...
  type PlayerEnvironmentBoundAction,
...
export class Sts2PlayerEnvironmentAdapter implements GameAdapter<PlayerEnvironmentRawState, ExecutableGameAction, GameExecutionResult> {
  ...
  async execute(action: ExecutableGameAction): Promise<GameExecutionResult> {
    ...
    let receipt: DecodedPlayerPayload<PlayerEnvironmentReceipt>;
    try {
      ...
      const submitFailure = safeMessage(error);
      try {
        // Query the idempotent ledger for this exact request. Never re-submit
        ...
        return unknown(
          requestId,
          invocation,
          "action_submit_transport_unknown",
          `${submitFailure}; receipt lookup failed: ${safeMessage(pollError)}`
        );
      }
    }
