...
import { HumanEnvironmentHttpError, HumanEnvironmentRestClient } from "./humanEnvironmentClient.js";
import type { HumanEnvironmentBoundAction, HumanEnvironmentCapabilities } from "./humanEnvironmentProtocol.js";
import { wrapHumanEnvironmentState, type HumanEnvironmentRawState } from "./rawState.js";
...
export interface HumanEnvironmentAdapterOptions {
...
export class Sts2HumanEnvironmentAdapter implements GameAdapter<HumanEnvironmentRawState, ExecutableGameAction, GameExecutionResult> {
  private readonly connector: HumanEnvironmentRestClient;
  private readonly control: GatewayControlSession;
  private capabilities?: HumanEnvironmentCapabilities;
  ...
  constructor(
    ...
    timeoutMs: number,
    private readonly options: HumanEnvironmentAdapterOptions,
    fetchImpl: typeof fetch = fetch,
    private readonly sleep: (ms: number) => Promise<void> = defaultSleep
  ) {
    this.connector = new HumanEnvironmentRestClient(baseUrl, timeoutMs, fetchImpl);
  ...
  async initialize(): Promise<void> {
    ...
    while (true) {
      try {
        ...
        if (!(error instanceof HumanEnvironmentHttpError) || remaining <= 0) throw error;
  ...
  describe(): AdapterDescriptor {
    ...
    return {
      adapterId: "sts2-human-environment",
      ...
      capabilities: {
        ...
        protocols: ["human_environment"]
      },
      negotiated: {
        protocol_mode: "human_environment",
  ...
  async readCurrentState(): Promise<HumanEnvironmentRawState> {
    ...
    return wrapHumanEnvironmentState({ snapshot: observed.raw });
  }

  async execute(action: ExecutableGameAction): Promise<GameExecutionResult> {
    if (action.kind !== "human_ui_action") {
      return rejected("action_authority_mismatch", "Human Environment Re accepts only current bound UI actions.");
    }
...
function assertIdentity(
  ...
  capabilities: HumanEnvironmentCapabilities
): void {
  if (observation.session.runtime_instance_id !== capabilities.host.runtime_instance_id) {
    throw new Error("Human Environment observation identity drifted during coherent read");
  }
  if (observation.session.environment_fingerprint !== capabilities.environment_fingerprint) {
    throw new Error("Human Environment environment identity drifted during coherent read");
  }
}
