...
export function projectPlayerVisibleRelic(relic: PlayerVisibleRelic) {
  return {
    ...
    cardPreviews: relic.card_previews.map(projectPlayerVisibleCard)
  };
}

export function projectPlayerVisibleState(shared: PlayerPersistentVisibleState): {
  ...
  return {
    run: {
      ...
      modifiers: shared.run.modifiers.map((modifier) => ({
        ...
        cardPreviews: modifier.card_previews.map(projectPlayerVisibleCard)
      }))
    },
    player: {
      ...
      orbs: [],
      statuses: [],
      relics: shared.player.relics.map(projectPlayerVisibleRelic),
      potions: shared.player.potions.map((potion) => ({
        ...
        cardPreviews: potion.card_previews.map(projectPlayerVisibleCard)
      })),
