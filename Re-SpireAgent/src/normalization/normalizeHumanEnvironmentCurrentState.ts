...
import { decodeHumanObservation } from "../integrations/sts2mcp/humanEnvironmentProtocol.js";
import {
  isHumanEnvironmentWrappedState,
  type HumanEnvironmentRawState
...
export function normalizeHumanEnvironmentCurrentState(
  rawInput: unknown,
  source: AdapterDescriptor,
  capturedAt = new Date().toISOString()
): StateEnvelope {
  const diagnostics = new DiagnosticsBuilder();
  const rawState: HumanEnvironmentRawState = isJsonObject(rawInput) ? rawInput : {};
  const wrapped = isHumanEnvironmentWrappedState(rawInput);
  if (!wrapped) {
    diagnostics.invalid(
      "$",
      rawInput,
      "Current Re production input must be a Human Environment wrapper"
    );
  }
  let decoded;
  try {
    if (!wrapped) throw new Error("Human Environment wrapper is missing");
  ...
  const currentState: NormalizedCurrentState = {
    ...
    sourceStateType: observation
      ? `human_environment:${observation.interaction.kind}`
      : "human_environment:invalid",
    ...
    surface: observation && built.status !== "invalid"
      ...
      : {
          kind: "unsupported",
          reason: "Human Environment snapshot failed strict decoding",
...
function invalidContext(raw: JsonObject): SemanticContext {
  return { kind: "unknown", reason: "Human Environment snapshot was invalid", observedTopLevelKeys: Object.keys(raw).sort() };
}
