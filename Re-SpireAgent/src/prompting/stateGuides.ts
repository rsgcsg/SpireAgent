...
export const SURFACE_GUIDES: Record<InteractionSurfaceKind, StateGuide> = {
  ...
  combat_pile_card_selection: guide("surface-combat-pile-card-selection", "Active surface is a source-bound combat-pile selection. Read purpose, source card, source and destination piles, destination position, overflow behavior, exact visible card instances, current selected set, and selection limits. Some sources commit after one pick; Dredge is a bounded multi-step toggle transaction that commits automatically at its exact required count. Do not infer one source card's business outcome from another source that happens to share this selector; choose only one Bridge-advertised opaque action per decision.", 4),
  ...
  generated_card_choice: guide("surface-generated-card-choice", "Active surface is a source-bound one-of-N generated card choice. Read purpose, sourceKind, destination, selectedCardCostPolicy, and overflowDestination instead of inferring business meaning from the shared card grid. The cards are offers, not current pile members. Choose one Bridge-advertised card, use an explicit skip when present, or return from peek mode before choosing."),
