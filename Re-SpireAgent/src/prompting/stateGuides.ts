...
export const SURFACE_GUIDES: Record<InteractionSurfaceKind, StateGuide> = {
  ...
  combat_pile_card_selection: guide("surface-combat-pile-card-selection", "Active surface is a source-bound combat-pile selection. Read purpose, source card, source and destination piles, destination position, exact visible card instances, and selection limits. The current Headbutt contract moves one chosen discard card to the top of the Draw Pile; choose only one Bridge-advertised opaque action."),
