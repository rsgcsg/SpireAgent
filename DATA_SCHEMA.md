...
- `LearningExperimentManifest` is the append-only G2 audit record at `data/runs/<runId>/learning-experiment-manifests.jsonl`. It binds one proposal, recorded baseline, optional same-slice overlay, evaluation, authority chain, environment fingerprint/scope, and invariant results. It may be emitted by `shadow-run` and is written only with explicit `--record-manifest`; it is not a promotion ledger and never changes proposal status.
