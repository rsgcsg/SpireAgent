...
The current G2 cloned-packet comparison admits only explicit low-risk `reason_policy` `deliberation_shaping` overlays with known protected targets. Candidate-template projection is retained as comparison/review tooling but is not an eligibility path. The first meaningful G3 policy remains unauthorized and must be a narrow, human-approved deliberation-shaping canary with ledger, rollback, and retrieval trace.
