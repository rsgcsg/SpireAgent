...
internal enum EnvironmentPermissionMode
{
    Strict,
    BalancedGray,
    DeveloperGray,
...
internal sealed record MigrationRiskRule(
    string RiskClass,
    IReadOnlyList<string> EligibleModes,
    int MinimumSuccesses,
    int SessionTtlSeconds);

internal sealed record MigrationPermissionCandidate(
    string SurfaceKind,
    string Operation,
    string RiskClass,
    IReadOnlyList<string> EligibleModes,
...
internal sealed record OperationPermissionBinding(
    string SurfaceKind,
    string Operation,
    string Tier,
    string GrantId,
...
/// </summary>
internal sealed record EncounterAuthorityCandidate(
    string SurfaceKind,
    string Operation,
    string SourceEvidence,
    bool RequiresExplicitNativeContract);

internal sealed class EnvironmentPermissionManager
{
    private readonly object _gate = new();
    private readonly string _runtimeEpoch;
    private readonly Func<DateTimeOffset> _clock;
    private readonly List<PermissionGrantRecord> _grantLedger = new();
    private readonly Dictionary<string, PermissionGrantRecord> _currentGrants =
    ...
    private EnvironmentPermissionMode _mode;
    private string? _activeEnvironmentDigest;
    private GatewayPatchInventoryInfo _lastPatchInventory =
        GatewayPatchInventoryInfo.Unavailable("Patch inventory has not been read.");

    public EnvironmentPermissionManager(
        string runtimeEpoch,
        EnvironmentPermissionMode mode = EnvironmentPermissionMode.BalancedGray,
    ...
    public static EnvironmentPermissionMode ParseMode(string? value)
    {
        if (string.IsNullOrWhiteSpace(value))
            return EnvironmentPermissionMode.BalancedGray;
        return value.Trim().ToLowerInvariant() switch
        {
            "strict" => EnvironmentPermissionMode.Strict,
            "balanced_gray" => EnvironmentPermissionMode.BalancedGray,
            "developer_gray" => EnvironmentPermissionMode.DeveloperGray,
            "migration_exploration" => EnvironmentPermissionMode.MigrationExploration,
            _ => EnvironmentPermissionMode.Strict
        };
    }

    public void ConfigureMode(EnvironmentPermissionMode mode)
    {
        lock (_gate)
        {
            if (_mode == mode)
        ...
        GatewayPatchInventoryInfo patchInventory)
    {
        lock (_gate)
        {
            ...
            foreach (ActionPermissionScope staticScope in game.Compatibility.ActionPermissionScopes)
            {
                ...
                if (_mode == EnvironmentPermissionMode.Strict)
                    continue;
                ...
                PermissionGrantRecord grant = EnsureSessionCanary(
                    candidate,
                    game,
                    bridge,
                    patchInventory,
            ...
            string[] inspectionCanaryKinds = readOnlyInspectionTrial
                ? game.Compatibility.InspectionCanaryKinds
                    .Concat(NativeOperationManifest.ImplementedInspectionKinds)
                    .Distinct(StringComparer.Ordinal)
                    .OrderBy(value => value, StringComparer.Ordinal)
                    .ToArray()
    ...
    public LiveObservation AdmitEncounter(
        ...
        IReadOnlyList<EncounterAuthorityCandidate>? authorityCandidates = null)
    {
        lock (_gate)
        {
            IReadOnlyList<EncounterAuthorityCandidate> candidates =
                authorityCandidates
                ?? Array.Empty<EncounterAuthorityCandidate>();
            if (_mode != EnvironmentPermissionMode.MigrationExploration
            ...
            foreach (IGrouping<(string SurfaceKind, string Operation),
                         EncounterAuthorityCandidate> group in candidates.GroupBy(
                         candidate => (candidate.SurfaceKind, candidate.Operation)))
            {
                EncounterAuthorityCandidate candidateDescriptor = group.First();
                if (!string.Equals(
                    ...
                    || group.Any(candidate =>
                       ...
                       && !OperationQualificationCatalog.IsExplicitContract(
                           candidateDescriptor.SurfaceKind,
                ...
                string[] sourceEvidenceDigests = group
                    .Select(value => StableIdentityHash.Text(value.SourceEvidence))
                    .Distinct(StringComparer.Ordinal)
                    .OrderBy(value => value, StringComparer.Ordinal)
                    .ToArray();
                if (baseOperationFingerprint == "unavailable"
                    ...
                    continue;
                string operationFingerprint = StableIdentityHash.Object(new
                {
                ...
                if (_blockedKeys.Contains(SourceKey(
                    ...
                    || HasApplicableScope(
                        ...
                        candidateDescriptor.Operation,
                        operationFingerprint)
                    || _currentGrants.TryGetValue(key, out PermissionGrantRecord? existing)
                ...
                string[] sourceEvidence = sourceEvidenceDigests
                    ...
                    .ToArray();
                MigrationPermissionCandidate encountered = candidate with
                {
    ...
    public bool AuthorizeExecution(
        OperationPermissionBinding expected,
        CompatibilityAssessment current,
        BoundOperationContract? contract = null)
    {
        lock (_gate)
        {
        ...
        GatewayCommandOutcomeEvidence response)
    {
        ...
        lock (_gate)
        {
            ...
            if (!_currentGrants.TryGetValue(key, out PermissionGrantRecord? current)
            ...
            if (response.Status == "completed" && response.Outcome == "confirmed")
            {
                if (current.Tier == "session_canary")
                {
                    ...
                    else if (!OperationQualificationCatalog.WitnessMatches(
                                 candidate.WitnessId,
                                 observedWitness))
                    {
                        Quarantine(
    ...
    public PermissionSystemInfo Snapshot()
    {
        lock (_gate)
        {
            bool candidatePolicyReady = EnvironmentMigrationPermissionPolicy.LoadError == null;
            ...
            var recentHistoricalGrantIds = _grantLedger
                ...
                .Select(record => record.GrantId)
                .ToHashSet(StringComparer.Ordinal);
            return new PermissionSystemInfo(
                1,
                candidatePolicyReady ? "active_session_scoped" : "candidate_policy_invalid_fail_closed",
                ModeName(_mode),
                _runtimeEpoch,
                EnvironmentMigrationPermissionPolicy.PolicyId,
                EnvironmentMigrationPermissionPolicy.PolicyDigest,
                DynamicSessionPromotionEnabled:
                    _mode != EnvironmentPermissionMode.Strict
                ...
                _grantLedger
                    ...
                    .Select(record => record with
                    {
                        Current = _currentGrants.TryGetValue(
                            ...
                            out PermissionGrantRecord? current)
                            && string.Equals(
                                current.GrantId,
                                record.GrantId,
                                StringComparison.Ordinal)
        ...
        string patchDigest)
    {
        var existing = scopes
            ...
            .ToHashSet(StringComparer.Ordinal);
        foreach (PermissionGrantRecord grant in _currentGrants.Values)
        {
    ...
    private bool EncounterEnvironmentEligible(
        GameBuildIdentity game,
        GatewayHostIdentity bridge) =>
        EnvironmentMigrationPermissionPolicy.LoadError == null
    ...
    private PermissionGrantRecord EnsureSessionCanary(
        MigrationPermissionCandidate candidate,
        GameBuildIdentity game,
        GatewayHostIdentity bridge,
        GatewayPatchInventoryInfo patchInventory,
        ...
        string admissionBasis = "installed_candidate_package")
    {
        ...
        if (_currentGrants.TryGetValue(key, out PermissionGrantRecord? current))
        {
            ...
            MigrationPermissionCandidate replacement = candidate with
            {
                EvidenceIds = candidate.EvidenceIds
    ...
    private void Promote(PermissionGrantRecord current)
    {
        DateTimeOffset now = _clock();
    ...
    {
        foreach ((string key, PermissionGrantRecord grant) in
    ...
    private PermissionGrantRecord AppendGrant(
        MigrationPermissionCandidate candidate,
        GameBuildIdentity game,
        GatewayHostIdentity bridge,
        GatewayPatchInventoryInfo patchInventory,
        ...
        string operationFingerprint)
    {
        ...
        return EnvironmentMigrationPermissionPolicy.LoadError == null
    ...
    private static ActionPermissionScope StaticScope(
        ...
        string operationFingerprint)
    {
        string grantId = "grant_static_" + StableIdentityHash.Text(
    ...
    private bool ReadOnlyInspectionTrialEligible(
        ...
        GatewayHostIdentity bridge,
        GatewayPatchInventoryInfo patchInventory) =>
        _mode == EnvironmentPermissionMode.MigrationExploration
    ...
    {
        OperationQualificationIdentity? qualification =
            OperationQualificationCatalog.Describe(surfaceKind, operation);
        if (qualification != null)
            return qualification.ContractDigest;

        NativeOperationManifestEntry? entry = NativeOperationManifest.Find(surfaceKind);
        NativeOperationManifestItem? declared = entry?.Operations.SingleOrDefault(value =>
            string.Equals(value.Operation, operation, StringComparison.Ordinal));
        if (entry == null || declared == null)
            return "unavailable";
        return StableIdentityHash.Object(new
        {
            manifestRevision = NativeOperationManifest.Revision,
            entry.Kind,
            declared.Operation,
            entry.Mechanism,
    ...
    private static string GrantId(
        ...
        int version,
        DateTimeOffset issuedAt) =>
        "grant_" + StableIdentityHash.Text(
    ...
    internal static string ModeName(EnvironmentPermissionMode mode) => mode switch
    {
        EnvironmentPermissionMode.Strict => "strict",
        EnvironmentPermissionMode.DeveloperGray => "developer_gray",
        EnvironmentPermissionMode.MigrationExploration => "migration_exploration",
        _ => "balanced_gray"
    };

}

internal static class EnvironmentMigrationPermissionPolicy
{
    private const string ResourceName =
        "STS2_MCP.Authority.migration-permission-policy.json";
    ...
    public static MigrationPermissionCandidate? Find(
        string surfaceKind,
        string operation)
    {
        OperationQualificationIdentity? identity =
            OperationQualificationCatalog.Describe(surfaceKind, operation);
        NativeOperationManifestEntry? entry = NativeOperationManifest.Find(surfaceKind);
        NativeOperationManifestItem? manifest = entry?.Operations.SingleOrDefault(value =>
            string.Equals(value.Operation, operation, StringComparison.Ordinal));
        MigrationRiskRule? rule = identity == null
            ? null
        ...
        return new MigrationPermissionCandidate(
            surfaceKind,
            operation,
            identity.RiskClass,
            rule.EligibleModes,
            ...
            new[]
            {
                $"operation-contract:{OperationQualificationCatalog.CatalogId}:{identity.ContractDigest}",
    ...
    private static LoadResult Load()
    {
        try
        {
            using Stream? stream = typeof(EnvironmentMigrationPermissionPolicy)
            ...
            return error == null
                ? new LoadResult(
                    ...
                    document.Rules,
                    null)
                : LoadResult.Failed(error, document.PolicyId, StableIdentityHash.Text(json));
        }
    ...
    private static string? Validate(IReadOnlyList<MigrationRiskRule> rules)
    {
        if (rules.Count == 0)
        ...
        foreach (MigrationRiskRule rule in rules)
        {
    ...
    private sealed record PolicyDocument(
        ...
        IReadOnlyList<MigrationRiskRule> Rules);
        ...
        string? Error)
    {
        public static LoadResult Failed(
            ...
            new(policyId, policyDigest, Array.Empty<MigrationRiskRule>(), error);
    }
}
