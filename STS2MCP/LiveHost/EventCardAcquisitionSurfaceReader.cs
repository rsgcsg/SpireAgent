...
/// </summary>
internal sealed class EventCardAcquisitionSurfaceReader : ILiveSurfaceReader
{
        ...
        GameBuildIdentity game)
    {
        ...
        VisibleCard[] cards = holders.Select(holder =>
        {
            ...
            cardIds[card] = cardId;
            return LiveContextReader.BuildCard(
                card,
                cardId,
        ...
        bool expectedSelected)
    {
        if (!IsCurrent(expectedScreen))
            return NativeInputResult.Rejected("screen_changed", "Event card acquisition is no longer current.");
        ...
        if (owner == null)
            return NativeInputResult.Rejected("event_owner_unavailable", "The audited event owner is unavailable.");
        ...
        IReadOnlyList<string> missing)
    {
        ...
        var completeness = new StateCompleteness(
            ...
            missing);
        return new LiveObservation(
            StableIdentityHash.Object(new { game.Version, context, unavailable, missing }),
            "degraded",
            context,
            unavailable,
            completeness,
            ...
            new[] { "event_card_acquisition_binding_unavailable" })
        {
            ...
            Diagnostics = new[]
            {
                GatewayDiagnostics.Create(
                    "gateway.surface.event_card_acquisition.binding_unavailable",
                    "error",
                    "surface",
                    "actions_suppressed",
                    "update_bridge",
