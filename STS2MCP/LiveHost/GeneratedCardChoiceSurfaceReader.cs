...
/// </summary>
internal sealed class GeneratedCardChoiceSurfaceReader : ILiveSurfaceReader
{
        ...
        CardModel expectedCard)
    {
        if (!TryCurrentActionable(expectedScreen, expectedSource, out Binding? binding, out string? error))
            return NativeInputResult.Rejected("choice_changed", error ?? "Generated-card choice is no longer current.");
        if (!binding!.Cards.Any(card => ReferenceEquals(card, expectedCard)))
            return NativeInputResult.Rejected("card_changed", "The advertised generated card is no longer in this choice.");
        ...
        if (holder == null)
            return NativeInputResult.Rejected("card_not_actionable", "The advertised generated card is no longer clickable.");
        ...
        return expectedSource switch
        {
            ...
            GeneratedCardChoiceSourceBinding.KnowledgeDemonCurseBinding curse =>
                NativeInputResult.Started(
                    ...
                    allowIntermediateStateChanges: true),
            _ => NativeInputResult.Rejected("source_not_supported", "Generated-card source is not supported.")
        };
    }
        ...
        string cardEntityId)
    {
        if (!entities.TryResolve(screenEntityId, out NChooseACardSelectionScreen? screen)
            ...
            || source == null
            || !SourceMatchesContext(source, LiveContextReader.Build(entities)))
        {
        ...
        NChoiceSelectionSkipButton expectedSkip)
    {
        ...
        {
            return NativeInputResult.Rejected("skip_changed", error ?? "Generated-card skip is no longer available.");
        }
        ...
        {
            return NativeInputResult.Rejected("skip_not_actionable", "The generated-card skip control is no longer enabled.");
        }
    ...
    private static bool SourceMatchesContext(
        GeneratedCardChoiceSourceBinding.ActiveBinding source,
        ILiveContext context) => source switch
    {
        GeneratedCardChoiceSourceBinding.LeadPaperweightBinding lead =>
    ...
    private static LiveObservation BindingUnavailable(
        ...
        => NativeUiFailClosedObservation.BindingUnavailable(
            ...
            "gateway.surface.generated_card_choice.binding_unavailable",
