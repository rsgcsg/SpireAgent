...
internal static class ExactEnvironmentPolicy
{
    private const string ResourceName =
        "STS2_MCP.LiveHost.exact-environment-policy.json";
        ...
        int? mainAssemblyHash)
    {
        ...
        if (loaded.Error != null)
        {
            return Disabled(
                ...
                testedFingerprints,
                version == null
                    ? "Game build identity is unavailable; mutation execution is disabled."
        ...
        IReadOnlyList<string> testedFingerprints)
    {
        ...
        return new CompatibilityAssessment(
            ...
            ObservationOnlySurfaceKinds: NativeOperationManifest.Entries
    ...
    private static PolicyLoadResult Load()
    {
        try
        {
            using Stream? stream = typeof(ExactEnvironmentPolicy).Assembly
            ...
            return validationError == null
                ? new PolicyLoadResult(
                    ...
                    null)
                : PolicyLoadResult.Failed(validationError, document.PolicyId, StableIdentityHash.Text(json));
        }
    ...
    private static string? Validate(IReadOnlyList<ExactEnvironmentPolicyEntry> environments)
    {
        ...
        HashSet<string> knownSurfaces = NativeOperationManifest.Entries
            .Select(entry => entry.Kind)
            .ToHashSet(StringComparer.Ordinal);
        HashSet<string> knownInspections = NativeOperationManifest.InspectionEntries
            .Select(entry => entry.Kind)
            .ToHashSet(StringComparer.Ordinal);
