...
/// </summary>
internal sealed class WoodCarvingsReplacementSurfaceReader : ILiveSurfaceReader
{
        ...
        GameBuildIdentity game)
    {
        ...
        if (context is not EventLiveContext eventContext
        ...
        CardModel expectedCard)
    {
        if (!IsCurrent(expectedScreen) || IsPreviewVisible(expectedScreen))
            return NativeInputResult.Rejected("screen_stage_changed", "Wood Carvings is no longer selecting a card.");
        ...
        if (holder == null || grid == null || BoundedCardSelectionFacts.IsSelected(expectedScreen, expectedCard))
            return NativeInputResult.Rejected("card_not_actionable", "The advertised Wood Carvings card changed before execution.");
    ...
    {
        if (!IsCurrent(expectedScreen) || !IsPreviewVisible(expectedScreen))
            return NativeInputResult.Rejected("screen_stage_changed", "Wood Carvings confirmation is no longer current.");
        if (expectedSelectedCards.Count != 1)
            return NativeInputResult.Rejected("selection_changed", "Wood Carvings no longer has exactly one selected card.");
        ...
        {
            return NativeInputResult.Rejected("wood_carvings_source_changed", "The selected card or exact Wood Carvings source changed before confirmation.");
        }
        ...
        if (confirm is not { IsEnabled: true } || !McpMod.IsNodeVisible(confirm))
            return NativeInputResult.Rejected("confirm_not_available", "The Wood Carvings confirmation control is no longer enabled.");
    ...
    {
        if (!IsCurrent(expectedScreen) || !IsPreviewVisible(expectedScreen))
            return NativeInputResult.Rejected("screen_stage_changed", "Wood Carvings preview is no longer current.");
        ...
        if (cancel is not { IsEnabled: true } || !McpMod.IsNodeVisible(cancel))
            return NativeInputResult.Rejected("cancel_not_available", "The Wood Carvings preview cancel control is no longer enabled.");
    ...
    internal static NativeInputResult StartDirectSelect(
        NativeEntityRegistry entities,
        string screenId,
        string cardId,
        ...
        string expectedReplacementDefinitionId)
    {
        ...
        foreach (string cardId in selectedCardIds)
        {
            if (!entities.TryResolve(cardId, out CardModel? card) || card == null)
                return NativeInputResult.Rejected("wood_carvings_selection_changed", "The exact selected Wood Carvings card no longer resolves.");
            selected.Add(card);
        }
    ...
    private static LiveObservation BindingUnavailable(
        ...
        NativeUiFailClosedObservation.BindingUnavailable(
            ...
            "gateway.surface.wood_carvings_replacement.binding_unavailable",
