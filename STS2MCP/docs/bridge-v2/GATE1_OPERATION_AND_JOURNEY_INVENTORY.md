...
|---|---|---|---|
| root menu -> standard run setup | `main_menu`, `singleplayer_menu`, `character_select` | exact Re canaries reached a real run; Preview.69 `run-20260728033638-xrvcre` crossed a fresh main menu through standard character select and continued for 100 ticks | provenance is unrecorded; first-run tutorial and abandon confirmation remain unsupported; operations are canary, not broadly qualified |
| root menu -> saved-run resume | `main_menu`, actionless `run_transition`, then current semantic Surface | Preview.69 final-MVID `run-20260728041630-2z58bz` confirmed Continue, crossed a saved floor-9 mount, resumed at rewards, and later completed the run boundary | provenance is unrecorded; the exact actionless mount is non-authorizing and may defer only typed `shared_visible_state`; other HUD failures remain fail closed |
...
| ordinary combat | `combat_turn` plus bounded child selectors | current combat-turn contract is qualified for the exact build; Preview.61 run `run-20260724045013-mgcq3a` completed the Neow's Fury play/select/confirm lifecycle; prior runs exercised Headbutt, Graveblast, Cleanse and Dredge | Preview.62 registry/source audit covers thirteen exact owner-bound sources without new provider branches; six new entries remain Organic-pending. Tutor's target-player ownership and every other unregistered selector stay fail closed |
...
| shop including removal | `shop_room`, `shop_inventory`, `deck_removal_selection` | source-audited canaries; removal is qualified; Preview.69 long runs repeatedly crossed ordinary room/inventory/purchase flows on one exact old MVID | unrecorded evidence does not promote tiers; repeat final-installed-MVID removal journey when naturally reached |
...
| run end -> menu | `game_over`, `main_menu` | Preview.69 old-MVID `run-...21602` and final-MVID `run-20260728041630-2z58bz` completed loss intro/summary/return lifecycles and stopped before a second game | unrecorded provenance; win/timeline diversity remains evidence debt |
...
1. Begin Gate 2 non-authorizing transaction-correlation and visible-information
   work without changing current permission tiers.
2. Run the non-authorizing exact-assembly audit on each exact build and review
   every unregistered caller before registry changes.
