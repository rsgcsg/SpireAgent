...
| formal state identities | non-authorizing identity fields | Preview.74 requires separate `semantic_state_id` and `authority_projection_id`; composite `state_id` binds both | source/tested only until Preview.74 is loaded; control histories and old shadows are no longer player/model state |
