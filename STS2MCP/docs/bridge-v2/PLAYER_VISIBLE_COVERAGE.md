...
| `deck_enchant_selection` | exact-source candidates for Self-Help Book, Symbiote and Kifuda; historical Self-Help Book lifecycle and pre-repair visible Symbiote selector were exercised | exact source task, target enchantment ID/amount, selected instances, overlay stage, semantic exact-card enchantment witness and independent run-deck post-state; unknown sources suppress actions | repaired Symbiote mutation and Kifuda remain pending exact-runtime evidence; explicit preview button, cancellation variants, Kifuda multi-select/skip, parent settlement, Mods, and future builds remain evidence debt |
...
| `shop_catalog` Inspection | read-only canary | preview.48 reads exposed fixed typed inventory; fresh Gate 1 evidence found and repaired context-only advertisement, then `run-20260722101242-cdmvev` crossed shop room/inventory/purchase/removal/close/proceed with no scope mismatch | additional ordinary shops and inventory mutations before qualification; catalog advertisement now requires the exact merchant binding and never grants purchase/navigation authority |
...
| formal state identities | non-authorizing identity fields | Preview.74 exact-runtime journeys used separate `semantic_state_id` and `authority_projection_id`; composite `state_id` binds both | current Preview.75 load remains pending; control histories and old shadows are no longer player/model state |
