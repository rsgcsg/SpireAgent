...
| `contract_instance_shadow` | diagnostic only, always `authorizing=false` | Preview.71 reports six explicit component-contract candidates with typed digests/boundaries and keeps 81 fallback rows as digest-free manifest hypotheses; later Preview.71 runs loaded the same identity without changing shadow authority | neither form is runtime semantic binding or permission; exact-environment operation authority remains separate, and Preview.72 needs a new loaded-runtime check |
