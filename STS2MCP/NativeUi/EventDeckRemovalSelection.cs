...
internal static class EventDeckRemovalSelection
{
        ...
        GameBuildIdentity game)
    {
        ...
        if (context is not EventLiveContext
        ...
        var surface = new EventDeckRemovalSelectionSurface(
            ...
            cards);
        IReadOnlyList<NativeUiActionDescriptor> actions = DescribeCommands(surface);
    ...
    {
        NativeInputResult? rejection = null;
        if (draft.Surface is not EventDeckRemovalSelectionSurface surface
            ...
            || source == null)
        {
            return rejection ?? NativeInputResult.Rejected(
        ...
        bool expectedSelected)
    {
        if (!IsCurrent(screen) || IsPreviewVisible(screen))
            return NativeInputResult.Rejected("screen_stage_changed", "Event removal is no longer selecting cards.");
    ...
    private static NativeInputResult StartPreviewCancel(NDeckCardSelectScreen screen)
    {
        if (!IsCurrent(screen) || !IsPreviewVisible(screen))
            return NativeInputResult.Rejected("screen_stage_changed", "Event removal preview is no longer current.");
        ...
        if (cancel is not { IsEnabled: true } || !McpMod.IsNodeVisible(cancel))
            return NativeInputResult.Rejected("cancel_not_available", "The preview return control is no longer enabled.");
        ...
        IReadOnlyCollection<CardModel> expectedSelected)
    {
        ...
        if (confirm is not { IsEnabled: true } || !McpMod.IsNodeVisible(confirm))
            return NativeInputResult.Rejected("confirm_not_available", "The event-removal confirmation is no longer enabled.");
    ...
    private static LiveObservation BindingUnavailable(
        ...
        string reason) => NativeUiFailClosedObservation.BindingUnavailable(
        game,
        context,
