...
- DeepSeek V4 Flash is prepared as the preferred P8 external provider through config, request shape, parser, output schema, conservative token/call/cost/timeout guards, usage/latency capture, and skipped/unavailable paths. It is injected as a P8 workspace decider, not the legacy live-prompt decider. `createLlmDecider()` ignores DeepSeek credentials; real P8 calls require `STS2_DEEPSEEK_API_KEY` plus explicit feature flags.
...
- `STS2_P8_WORKSPACE_MAX_SHADOW_CALLS` defaults to `1`; hard token/cost/call budget excess records a skipped shadow decision and does not fail eval.
