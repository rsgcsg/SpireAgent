...
- `npm run learning:proposals -- shadow-preflight --run-id <runId> --id <proposalId> --transition-id <transitionId>` performs no provider call. It currently requires explicit LLM product authority, complete exact organic environment scope, executor-logged provenance, a called workspace-shadow baseline, and the legacy `presentation_only` label before `shadow-run` may spend a provider call. This current G2 restriction does not authorize G3; ADR-0006 requires mutation-surface/decision-influence semantics, source-resolved eligibility, and truthful selection provenance before another pair can advance the gate.
...
- `DecisionAuthorizationRecord` now distinguishes deliberation owner, selection source, authorization source, execution source, plan origin, authority mode/level, and delegated skill. Historical `chosenBy` remains readable but incomplete; product authority mode is `unknown` unless explicitly configured. Before G3, a `SelectionResolutionRecord` must preserve LLM/local proposal, final candidate, and any local safety override; historical proposal/final mismatches are a conservative exclusion from LLM-selection evidence, not a raw-data rewrite.
- Fresh `LearningProposal` records must declare `ProposalBehaviorImpact`. `unclassified` historical or incomplete records remain readable but cannot become actionable. G2 cloned shadow overlays currently accept only `presentation_only` as an offline comparison restriction; this does not mean that a pre-decision overlay is decision-neutral or G3-promotable.
...
The current role labels are an interim read-only display. Before G3, replay, proposal generation, and manifests must derive them from one shared structured observation of workspace call, proposed selection, final selection, execution, and selection-provenance consistency. Environment/provenance eligibility and an evidence role are both necessary context, not promotion authorization.
