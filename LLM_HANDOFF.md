...
- G2 seed eligibility now records observed evidence role. `llm_primary` is a declared product mode, not proof of actual LLM execution; direct workspace shadows can coexist with local fallback execution. Local fallback/scaffold/mechanical smoke observations remain draft-only, while a called workspace provider or LLM selection is required before future actionable review. No promotion path exists.
- Current repeated provider-backed dry-run signals are `candidate_future:card_flow` and remain `candidate_shaping`; they are not a reason to start G3. P9.5 has a documented candidate-template overlay eligibility mismatch: do not bypass it by mislabeling candidate changes as presentation-only.
