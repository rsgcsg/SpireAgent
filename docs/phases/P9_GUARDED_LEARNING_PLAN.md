...
- Current design debt: the offline overlay policy presently requires `presentation_only`, while generated candidate-template proposals are correctly classified as `candidate_shaping`. Therefore candidate-template overlay language is aspirational rather than currently executable. Do not relabel a candidate change as presentation-only to bypass this; resolve it as a separate bounded G2 decision before any candidate-template shadow experiment.
