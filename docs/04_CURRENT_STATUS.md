...
- Fresh G2 organic evidence now exists in `run-mrfq2vw1-aap49m`: eleven valid direct-shadow calls across combat, shop, event, rewards, and card selection under exact environment scope and `llm_primary` mode. All have `finishReason=stop`, `failureBucket=none`, and no output-cap hit. Three are a deliberately fixed `maxShadowCalls=3` direct-shadow sub-window; other records use earlier budget profiles and must not be mixed into a future pair. A fresh card-reward reason, "Patches block deficit now, slight deck bloat.", was labeled `missing_tradeoff` despite explicitly naming benefit and cost; it is a detector counterexample, not evidence to hand-author a reason-policy patch. These records are baselines/counterexamples only, not paired policy results or promotion evidence.
...
2. collect fresh authority-complete and environment-complete organic paired shadow evidence only when a repeated, scoped, low-risk presentation deficiency survives counterexample review; do not manufacture a proposal merely to obtain a pair
