...
- Fresh G2 organic evidence now exists in `run-mrfq2vw1-aap49m`: twenty valid direct-shadow calls across combat, shop, event, rest, rewards, and card selection under exact environment scope and `llm_primary` mode. They have `finishReason=stop`, `failureBucket=none`, and no output-cap hit; one combat outcome used retry=1 and therefore cannot be mixed with no-retry pair evidence. Fixed `maxShadowCalls=3` sub-windows are separately inspectable; other profiles must not be mixed into a future pair. A card-reward reason that explicitly named benefit and deck-bloat cost was labeled `missing_tradeoff`, so it remains a detector counterexample. One rest reason stated upgrade value and high HP but omitted the heal-for-upgrade cost; it remains a single weak presentation-deficiency candidate after a bounded follow-up window found no comparable repeat. It is not an actionable proposal. These records are baselines/counterexamples only, not paired policy results or promotion evidence.
