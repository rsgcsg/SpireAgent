...
- Replay/eval/review now also expose a separate `liveAppliedRollout` summary. This is a current live-facts view over transitions that actually applied additive live decisions. It reports final LLM-selected execution separately from legacy `chosenBy="llm"` records with no resolution telemetry, local safety overrides, fallback count, decision-class counts, provider-source counts, prompt mode counts, and live invalid/error/missing-candidate signals.
...
- Evidence slices also report proposal-seed eligibility separately from general visibility:
  - `proposalEvidence.proposalSeedEligible`: a clean source-resolved workspace-provider or final LLM-selection observation under exact organic executor-logged scope. This is structural proposal-seed eligibility only, never promotion authority.
  - `proposalEvidence.proposalSeedExcluded`: transitions still visible in replay/eval/review but excluded from actionable proposal seeds, including legacy selection provenance, console/debug/fixture, provider failure, and incomplete environment scope.
  - `proposalEvidence.proposalSeedExclusionReasons`: the reason a transition was excluded, such as `selection_provenance_not_recorded`, `console_debug_or_fixture`, `missing_environment_scope`, `environment_fingerprint_incomplete`, `environment_scope_partial`, `human_observed`, `snapshot_only`, or `unknown`.
  Source roles are additive: a called workspace-provider observation and a resolved final-selection observation may coexist. `promotionUseAllowed` remains `false` for every P9-G2 slice. These fields are read-only labels; they do not implement promotion, delete historical data, or change live behavior.
...
- `npm run learning:proposals -- shadow-preflight --run-id <runId> --id <proposalId> --transition-id <transitionId>` performs no provider call. It requires explicit LLM product authority, complete exact organic environment scope, executor-logged provenance, a called workspace-shadow baseline, a complete provider experiment fingerprint, source-resolved proposal-seed eligibility, non-empty canonical protected targets, and a low-risk `reason_policy` `deliberation_shaping` overlay. The selected transition must be named by the proposal source and evidence records and match its declared fingerprint/provenance; proposal-embedded role labels are never sufficient. This G2 restriction does not authorize G3.
...
- Same-slice evaluation also requires matching secret-free provider experiment identity (provider source, provider, model, response mode, thinking mode, output cap, retry/recovery topology). A clean one-off pair is review evidence only, never an applied policy or a reason-quality success metric.
...
- `DecisionAuthorizationRecord` now distinguishes deliberation owner, selection source, authorization source, execution source, plan origin, authority mode/level, and delegated skill. Fresh `SelectionResolutionRecord` preserves LLM/local proposal, final candidate, and any local safety override; historical proposal/final mismatches are conservatively excluded and `chosenBy`-only records remain `not_recorded`, not raw-data rewrites.
- Fresh `LearningProposal` records must declare `ProposalBehaviorImpact`. `unclassified` historical or incomplete records remain readable but cannot become actionable. G2 cloned shadow overlays accept only low-risk `reason_policy` `deliberation_shaping` comparison; this is an honest pre-decision influence label and still does not authorize G3.
...
Replay, proposal generation, shadow preflight, manifests, and live-rollout reporting derive source roles from the shared structured observation of workspace call, proposed selection, final selection, execution, and selection-provenance consistency. Environment/provenance eligibility and an evidence role are both necessary context, not promotion authorization.
