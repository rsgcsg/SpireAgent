...
- Proposal evidence records preserve a source-resolved `evidenceRoles[]` vector and an `evidenceEligibility` snapshot from the shared P9-G2 classifier. Exact organic environment scope alone is not an LLM-learning claim: `local_fallback_observation`, `local_scaffold_observation`, `local_safety_guard_observation`, and `local_mechanical_observation` remain visible as draft-only observations. A clean `llm_selected_execution` or `workspace_shadow_provider` source may satisfy proposal-seed eligibility, but neither role enables promotion.
- Historical `chosenBy="llm"` without a `SelectionResolutionRecord` is `legacy_selection_provenance_not_recorded`, not LLM-selection proof. A detected historical proposal/final mismatch is `selection_provenance_mismatch_excluded` and cannot satisfy proposal-seed eligibility.
...
- `LearningProposalShadowOverlayPlan` reports whether a proposal is eligible for an *offline* shadow comparison. Eligibility requires actionable `pending_review` status, `riskLevel="low"`, exact organic source-resolved evidence, explicit non-empty known protected targets, and a bounded `proposedPatch.shadowOverlay` shape. The caller must resolve one replayed run/transition named by both the proposal source lists and evidence record; its exact environment fingerprint/provenance must match the proposal scope. A proposal-embedded eligibility snapshot alone is never authorization. It does not imply approval, promotion, or runtime applicability.
- The current overlay boundary admits only a narrow `reason_policy` `deliberation_shaping` comparison. Candidate-template/generation changes are `candidate_shaping` and remain outside this first canary path. A cloned packet is isolated from runtime, but a future pre-decision policy is never called decision-neutral merely because the current comparison is offline.
- `P9ShadowWorkspaceOverlay` is an optional, in-memory `DeliberationPacket` field created only by `p9_shadow_workspace_applicator_v1`. The normal scaffold builder does not create it; no transition write, candidate reorder, validation/execution change, or stable mutation is permitted. A separate same-slice provider call is explicitly preflighted and never becomes a live call.
...
- P9.5B defines a read-only `p9_shadow_workspace_evaluation` surface for a paired baseline/overlay shadow outcome. The pair must share transition id, revision tag, budget window, provider profile, allowed-candidate hash, candidate-future-facts hash, and their respective baseline/overlay prompt hashes. G2.3 adds `ProviderExperimentFingerprint` to the manifest layer: exact applicability additionally requires provider source/provider/model/response-mode/thinking/output/retry/recovery-attempt identity, without storing credentials or raw provider output.
...
- `LearningExperimentManifest` is the append-only G2 audit record at `data/runs/<runId>/learning-experiment-manifests.jsonl`. It binds one proposal, recorded baseline, optional same-slice overlay, evaluation, authority chain, environment fingerprint/scope, source-resolved baseline/overlay evidence roles, provider experiment identity, exact-identity applicability, and invariant results. `shadow-run` preflight requires a clean called workspace-shadow baseline, exact organic environment, explicit authority mode, complete provider experiment fingerprint, and a `deliberation_shaping` proposal. It may be emitted only with explicit `--record-manifest`; it is not a promotion ledger and never changes proposal status.
- The reader emits a `LearningExperimentManifestStoreDigest` with content hash, legacy-record count, malformed-line count, and read status. Legacy or malformed entries remain visible as store-health diagnostics and cannot silently count as current G2.3 evidence.
- P9-G3A adds disabled change-kernel records at `data/runs/<runId>/policy-artifacts.jsonl`, `policy-change-events.jsonl`, and `policy-rollback-snapshots.jsonl`. `PolicyArtifact` is versioned but always `disabled` or `quarantined` and names both exact environment scope and the secret-free provider experiment fingerprint hash; `PolicyChangeEvent` is append-only; `PolicyRollbackSnapshot.rollbackMode` is `simulation_only`; `PolicyRetrievalTrace.result` is `disabled_no_activation`, `quarantined`, or `scope_mismatch`. Reader output includes malformed-line counts. These records are lifecycle-safety fixtures, not a promotion ledger or stable store.
...
### G2 Selection Resolution and Evidence Roles

`DecisionAuthorizationRecord` alone is not sufficient when a safety guard changes a valid proposed candidate after selection. Fresh executor-logged transitions now emit `SelectionResolutionRecord`, containing proposed candidate/source, final candidate/source, resolution kind/reason, validation result, candidate legality, and LLM-selection-evidence eligibility. `DecisionAuthorizationRecord.selectionSource` is derived from this final resolution when present.

Older transitions are never rewritten. If historical LLM proposal and final candidate IDs differ, replay derives `selection_provenance_mismatch_excluded`; if the old record only says `chosenBy="llm"`, it is `legacy_selection_provenance_not_recorded`. Neither can make an LLM-selection evidence claim.
...
Historical proposals without this field remain readable as `unclassified`, but cannot become actionable pending review. `presentation_only` is reserved for post-decision display/observability with no possible decision influence. Prompt/workspace/context guidance is `deliberation_shaping`; candidate content/generation is `candidate_shaping`; routing/authority, action, and hard-shell surfaces remain higher-risk.

The current G2 cloned-packet comparison admits only explicit low-risk `reason_policy` `deliberation_shaping` overlays with known protected targets. Candidate-template projection is retained as comparison/review tooling but is not an eligibility path. G3-A may record disabled run-local `PolicyArtifact`, `PolicyChangeEvent`, `PolicyRollbackSnapshot`, and dry-run `PolicyRetrievalTrace` records; these never activate, retrieve, or write stable policy. The first meaningful G3-B/C policy remains unauthorized and must be a narrow, human-approved deliberation-shaping canary with qualified evidence, lifecycle ledger, rollback, and active retrieval trace.
...
`EvidenceSliceReader` is a read-only report layer. It can narrow a replay view by decision class, revision, budget window, environment fingerprint, authority mode, capture provenance, and whether a workspace shadow call occurred. A filtered slice records its filters and matched transition IDs so a same-scope comparison is inspectable. The shared `TransitionEvidenceRoleClassification` reports additive source roles rather than selecting one by precedence. Its one fail-closed `proposalSeedEligible` result requires exact organic environment plus a clean source-resolved workspace-provider or resolved LLM-final-selection observation. `promotionUseAllowed` remains `false` until a future G3 gate exists.
