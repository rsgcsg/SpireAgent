...
## 2026-07-11 P9-G2 Exact Organic Baseline And Counterexample Review

Historical/debug note only. Canonical phase and next step are in [docs/04_CURRENT_STATUS.md](docs/04_CURRENT_STATUS.md).

- A natural Defect run, `run-mrg5a33g-ko7axj`, was started through normal menu flow after an ordinary loss; no console, fixture, or gameplay mod was used.
- Fresh transitions declare exact environment fingerprint `95503199bfa79d2c` (build `24032229`, beta channel, declared empty mod list, adapter/fact/agent identities) and later child windows declare `decisionAuthority.authorityMode=llm_primary`.
- `transition-000120-agent-mrg5imyb-m7pfyg` is the first clean G2 combat baseline with all required observation layers: real `combat:llm_required` live selection, allowed candidate match, executor action, called shadow workspace provider outcome, provider `stop`, no cap hit, no retry, and no provider failure.
- Its `missing_tradeoff` smoke signal is not accepted as a proposal seed yet. CandidateFuture tradeoff/resource/future-risk/survival cues were preserved; the reason says "Apply Weak to reduce damage, conserve energy for later." This is insufficient to claim a missing tradeoff without a repeated same-scope positive and counterexample review.
- `transition-000063-agent-mrg5dvgs-5ofdk5` also shows why selection resolution matters: its shadow reason describes shadow candidate `shop-0`, while live execution selected allowed candidate `shop-3`. This is a recorded shadow/live disagreement, not execution mismatch or a basis for rewriting the live reason contract.
- No proposal, same-slice overlay, manifest, stable promotion, stable write, whitelist change, or `.env.local` edit was made. `memory/legacy-finalize-audit.jsonl` changed only because the old run finalized under an explicitly blocked/audited legacy finalize path; it remains a local runtime artifact.
